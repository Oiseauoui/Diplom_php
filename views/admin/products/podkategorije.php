...
<?php
    ...
    if(!empty($_GET['error']) && $_GET['error'] == '1'){
     echo   '<h3 style="color:red">Категория успешно добавлена!</h3><br>';
    }
    ...
    if(!empty($_GET['error']) && $_GET['error'] == '5'){
     echo   '<h3 style="color:red">Не удаляется, вы должны сначала удалить продукты и подкатегории!</h3><br>';
    }
...
?>
    ...
    <form class="form" action="<?php echo ADMIN_URL . 'products/addSubcategory'; ?>" method="post">
...
<div class='col-xs-12 col-sm-6 col-md-7'>
    <table class="table-responsive mt15" border="1">
        ...
        foreach ($this->podkategorije as $subCategory) {
            ...
            echo '<td><a href="' . ADMIN_URL .'products/selectedSubcategory/' . $subCategory['sub_category_id'] . '" title="Изменить категорию">Изменить</a></td>';
            echo '<td><a href="' . ADMIN_URL .'products/deleteSubcategory/' . $subCategory['sub_category_id'] . '" title="Удалить категорию" onclick="return confirm(\'Вы уверены, что хотите удалить?\');"  ><img src="' . URL . 'images/delete.png" /></a></td>';
            echo '</tr>';
        }
        ?>
    </table>
